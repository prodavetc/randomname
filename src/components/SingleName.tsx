...
const eliminarElemento = (list: string[], setList: (list: string[]) => void, index: number) => {
  const newList = [...list];
...
const SingleName = ({ item, index }: SingleNameProps) => {
  ...
  return (
    <span
      className="inline-flex items-center px-2 py-1 mt-2 me-2 text-sm font-medium text-purple-800 bg-purple-100 rounded dark:bg-purple-900 dark:text-purple-300"
    >
      {item.replace(',', '')}
      <button
